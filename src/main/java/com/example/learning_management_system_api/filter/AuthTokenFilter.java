...
public class AuthTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            ResponseVO<?> errorResponse =
                    ResponseVO.error(
                            HttpServletResponse.SC_UNAUTHORIZED, "Unauthorized: " + e.getMessage() + " (Invalid or expired token)");
    ...
    private boolean isBypassUrl(HttpServletRequest request) {
        ...
        if(requestURI.contains("/update-password"))
            return false;
        return requestURI.startsWith("/public") || requestURI.contains("/api/auth")||requestURI.contains("/api/purchases/callback");
    }
