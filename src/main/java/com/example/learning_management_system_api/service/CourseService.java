...
public class CourseService implements ICourseService {
    ...
    public CourseService(CourseRepository courseRepository, CourseMapper courseMapper, StudentMapper studentMapper, EnrollRepository enrollRepository, ReviewRepository reviewRepository, ReviewMapper reviewMapper) {
    ...
    public PageDto getAllCourse(int page, int limit, String courseName, String categoryName, Double price, Long instructorId){
        Pageable pageable = PageRequest.of(page, limit);
        Page<Course> coursePage = courseRepository.findAll(courseFilter(courseName, categoryName, price, instructorId), pageable);
        List<CourseResponseDto> result = coursePage
                .getContent()
                .stream()
    ...
    public PageDto getStudentOfCourse(Long id, int page, int limit) {
        ...
        List<StudentResponseDto> studentList = enrollPage.getContent()
                .stream()
                .map(Enroll::getStudent)
                .map(studentMapper::toResponseDto)
                .toList();
        return new PageDto(enrollPage.getNumber(),
                enrollPage.getSize(),
    ...
    public PageDto getReviewOfCourse(Long id, int page, int limit) {
        ...
        List<ReviewResponseDto> reviewList = reviewPage.getContent()
                .stream()
    ...
    public static Specification<Course> courseFilter(String courseName, String categoryName, Double price, Long instructorId) {
        return (root, query, criteriaBuilder) -> {
            List<Predicate> predicates = new ArrayList<>();

            if (courseName != null) {
                predicates.add(criteriaBuilder.like(criteriaBuilder.lower(root.get("name")), "%" + courseName.toLowerCase() + "%"));
            }

            if (categoryName != null) {
                Join<Course, Category> categoryJoin = root.join("category");
                predicates.add(criteriaBuilder.like(criteriaBuilder.lower(categoryJoin.get("name")), "%" + categoryName.toLowerCase() + "%"));
            }
    ...
    public CourseResponseDto updateCourseStatus(Long courseId, String status) {
        ...
        if ("APPROVED".equalsIgnoreCase(status)) {
            ...
            throw new IllegalArgumentException("Invalid status value: " + status +" (must be either 'APPROVED' or 'REJECTED')");
        }
