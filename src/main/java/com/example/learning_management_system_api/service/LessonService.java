...
public class LessonService implements ILessonService{
    ...
    public LessonService(LessonRepository lessonRepository, CourseRepository courseRepository, LessonMapper lessonMapper, PurchaseRepository purchaseRepository) {
    ...
    public LessonResponseDto createLesson(LessonRequestDto requestDTO) {
        Course course = courseRepository.findById(requestDTO.courseId())
                .orElseThrow(() -> new NoSuchElementException("Course not found with ID: " + requestDTO.courseId()));
        if (lessonRepository.existsByNameAndCourseId(requestDTO.name(), requestDTO.courseId())) {
            throw new DuplicateKeyException("There is already a lesson with the name '" + requestDTO.name() + "' in course ID: " + requestDTO.courseId());
        }
    ...
    public List<LessonResponseDto> getAllLessons(Long courseId, String name) {
        ...
        List<Lesson> lessonList = lessonRepository.findByCourse_IdAndNameContaining(courseId, name==null?"":name);
    ...
    public LessonResponseDto updateLesson(Long id, LessonRequestDto requestDTO) {
        ...
        if (requestDTO.name() != null) {
            if (lessonRepository.existsByNameAndCourseId(requestDTO.name(), lesson.getCourse().getId())) {
                throw new DuplicateKeyException("There is already a lesson with the name '" + requestDTO.name() + "' in course ID: " + requestDTO.courseId());
            }
        }
    ...
    public void checkGetPermission(Course course) {
        ...
        if (authentication != null && authentication.getPrincipal() instanceof CustomUserDetails customUserDetails) {
            ...
            // Check if this user have purchased
                ...
                List<Purchase> purchaseList = purchaseRepository.findDistinctCoursesByStudent_User_IdAndIsPaidTrue(userId);
                List<Course> boughtCourse = purchaseList.stream()
        ...
        if (!isAllowed) {
            throw new AppException(403,"Access denied. You cannot access this course because you are neither the owner of this course nor have you purchased it.");
        }
    }
}
