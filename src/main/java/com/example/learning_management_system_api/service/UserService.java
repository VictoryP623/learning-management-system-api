...
public class UserService implements IUserService{
    ...
    public UserResponseDto updateUser(Long id, UserRequestDto userRequest) {
        User user = userRepository.findById(id).orElseThrow(()-> new NoSuchElementException("User with id "+id+ "is not found"));
        userMapper.updateUserEntity(userRequest, user);

        CustomUserDetails userAccount = (CustomUserDetails) SecurityContextHolder.getContext().getAuthentication().getPrincipal();

        if (!Objects.equals(userAccount.getUserId(), user.getId()) && userAccount.getAuthorities().stream().noneMatch(auth -> auth.getAuthority().equals("ROLE_Admin"))){
    ...
    public PageDto getUsersByRole(UserRole role, Pageable pageable) {
        ...
        List<Object> data = userPage.getContent()
                .stream()
